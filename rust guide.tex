...
\newpage
\section{References}
All variables can be passed as a reference by prefixing with a \lstinline[language=Rust]{&}, Rust will often automatically deference pointers if the value implements \lstinline[language=Rust]{Copy} which makes this quite painless to use:
\begin{lstlisting}[language=Rust,frame=single]
let foo = 10;
print(foo);
print_ref(foo);
