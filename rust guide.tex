...
Rust is designed so that the compiler can guarantee some level of correctness in regards to variable creation and access, and as such it will not compile code it can't predict; however this is sometimes necessary and so you can use the \lstinline[language=Rust]{unsafe} keyword to mark a block of code that you're guaranteeing will be safe.

\subsection{Core differences}
In Rust...
\begin{itemize}
	\item lines have to end with a semicolon.
  \item there is an official and very opinionated linter - it should always be used.
...
fn change(value: &mut i32) {
	*bar += 1; 
	//Has to be dereferenced as Rust doesn't perform 
	//automatic deference with operators (as you 
	//might want to do pointer maths, it is
...
impl TraitA { //methods only, can not 
...
Statements and expressions must have a semicolon at the end except if it's the last
...
\lstinline[language=Rust]{Box} allows you to store values on the heap, this is sometimes necessary as the stack can only hold values with a known (at compile time) size, as the \lstinline[language=Rust]{Box} is just a pointer it has a known unlike, for example, lambdas.
\newpage
\section{Modules}
...
You can also do the rough equivalent of  \lstinline[language=Kotlin]{var!!} with both \lstinline[language=Rust]{Option} and \lstinline[language=Rust]{Result} by using \lstinline[language=Rust]{var.unwrap()} and \lstinline[language=Rust]{var.expect("some message")}. Both methods will crash the app if it's \lstinline[language=Rust]{Err}/\lstinline[language=Rust]{None}, \lstinline[language=Rust]{expect()} will also write the message to the console.
\newline
